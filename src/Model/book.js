class Book {
  ...
  async addBook(title, author, isbn, available_quantity, shelf_location) {
    ...
    try {
      const [result] = await this.db
        .promise()
        .query(sql, [title, author, isbn, available_quantity, shelf_location]);
  ...
  async updateBook(
    ...
    const sql =
      "UPDATE books SET title = COALESCE(?, title), author = COALESCE(?, author), isbn = COALESCE(?, isbn), available_quantity = COALESCE(?, available_quantity), shelf_location = COALESCE(?, shelf_location) WHERE id = ?";
    try {
      await this.db
        .promise()
        .query(sql, [
  ...
  async borrowBook(userid, bookid, date) {
    const sql =
      "INSERT INTO user_books (user_id, book_id , due_date) VALUES ( ? , ? , ? )";
    try {
